...
class CursoController extends Controller
{
    ...
    public function obtenerCursos($alumno_id, $ciclo)
    {
        ...
        $cuentaMateriasInscritas = Lista::where([
            ['IdAlumno', '=', $alumno_id],
            ['ciclo', '=', $ciclo],
            ['status', '=', 'Inscrito']
        ])->count();
        $MateriasInscritas = MateriaInscrita::where([
            ['IdAlumno', '=', $alumno_id],
            ['ciclo', '=', $ciclo]
        ])->get();
    ...
    public function saveSeleccionCursos($alumno_id, $curso_id, $ciclo)
    {
        ...
        if ($conteoMateriasInscritas < 3) {
            ...
            if ($disponible >= 1) {
                $YaCursada = Lista::where([
                    ['IdAlumno', '=', $alumno_id],
                    ['ciclo', '=', $ciclo],
                    ['IdCurso', '=', $curso_id],
                ])->count();
                if ($YaCursada == 0) {
                    $ListaMateriasInscritas = MateriaInscrita::where([
                        ['IdAlumno', '=', $alumno_id],
                        ['ciclo', '=', $ciclo]
                    ])->get();
                    foreach ($ListaMateriasInscritas as $ListaMateriasInscrita) {
                        if (
                            $ListaMateriasInscrita->hora1 == $curso->hora1
